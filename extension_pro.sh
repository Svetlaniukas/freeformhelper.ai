...
echo "🧩 СОЗДАЕМ РАСШИРЕНИЕ-УБИЙЦУ (PRO VERSION)..."

# ==========================================
# 1. ОБНОВЛЯЕМ API НА СЕРВЕРЕ (Разрешаем CORS)
# Это критически важно, иначе расширение не сможет подключиться к серверу.
# ==========================================
...
cat > extension/manifest.json << 'EOL'
{
  ...
  "version": "2.0",
  "description": "Select text on Canvas/Blackboard and humanize it instantly.",
...
# ==========================================
# 3. ИНТЕРФЕЙС РАСШИРЕНИЯ (POPUP.HTML)
# Темный стиль, как на сайте
# ==========================================
...
echo "✅ РАСШИРЕНИЕ PRO СОЗДАНО!"
echo "✅ API ОБНОВЛЕН (CORS РАЗРЕШЕН)"
echo "------------------------------------------------"
echo "СЛЕДУЮЩИЕ ШАГИ:"
echo "1. Выполни: git add . && git commit -m 'Add Extension Pro' && git push -u origin main --force"
echo "2. Подожди пока Render обновится (чтобы заработал CORS)."
echo "3. Загрузи папку 'extension' в Chrome (chrome://extensions -> Load Unpacked)."
